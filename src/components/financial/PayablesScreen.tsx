...
import { mockPayables } from '@/data/financialMockData';
...
interface PayablesScreenProps {
  franchiseId: string;
}

export const PayablesScreen = ({ franchiseId }: PayablesScreenProps) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<AccountStatus | 'all'>('all');

  const filteredPayables = mockPayables.filter(account => {
    const matchesSearch = 
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <StatusTimelineModal
                            ...
                            subtitle={`Fornecedor: ${account.entityName}`}
